...
async def insert_data(id_tread, content, role, message_datetime=None):
    try:
        ...
        if message_datetime:
            created_at = datetime.strptime(message_datetime, '%Y-%m-%dT%H:%M:%S')
        else:
            created_at = None  # PostgreSQL установит текущее время
        print(role, id_tread, content, created_at)
...
async def load(data):
    ...
    for record in data:
        ...
        for row in record['messages']:
            ...
            #print(row['content'])
            #print(row['datetime'])
            if row['role'] == 'girl': #считаем это за ответ модели
                role = 'assistant'
            else:
                role = 'user'
